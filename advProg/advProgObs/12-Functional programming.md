...
Iterators are object that manage the operations on each one of the element in a list and the logic to determine the end of the sequence.

Iterators are lazy, without a function with uses their return value they don't apply the actions. The function to convert it in a vector is `.collect()`
