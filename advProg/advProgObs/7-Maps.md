...
pub fn maps_options(){  
    ...
    let numbers: Vec<u32> = str_numbers  
        .iter()  
        // does not compile  
        // .map(|str_number| str_number.parse::<u32>())        // must use `flat_map`        //  https://doc.rust-lang.org/std/iter/trait.Iterator.html#method.flat_map        .flat_map(|str_number| str_number.parse::<u32>())  
        .collect();  
    println!("{:?}",numbers);  
}
```