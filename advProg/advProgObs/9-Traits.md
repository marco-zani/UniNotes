...
that reduce code duplication, so instead of having to write  
    Option<i32>        Some(i32)    Option<usize>        Some(usize)one writes  
    Option <T>        Some(T)  
Generics have long been studied in logics and formal languages, the  
    Tof a generic declaration is called  
    Type Parametermuch like in a function there are parameters  
    pub fn add(x, y)in a type declaration (be it a struct / enum / function singnature), there are  
    Type Parametersfor example:  
*/  
...
Much like when calling a function we supply arguments for the parameter  
    add(1,2)we need to supply type arguments to stuff with type parameters.  
...
pub fn generics_example() {  
    ...
    // Explicitly specified type parameter `char` to `generic()`.    generic::<char>(SGen('a'));  
...
// that describe the behaviors of the types that implement this trait,    // which in this case is `fn summarize(&self) -> String`.    
	fn summarize(&self) -> String;  
    //We don't give the function body to `summarize`,  
    // because we want the `struct`s that implement this trait    // to implement their own `summarize` function.  
    // However, we give a default implementation to function `say_hello()`,    // so that the concrete type can choose to use this default implementation,    // or implement their own.    
...
pub fn traitexample(){  
    ...
    // hello / hello | hello / yello | yello / hello | yello / yello |    NewsArticle::say_hello();  
    Tweet::say_hello();  
}  
...
fn example_derivable() {  
    ...
    //println!("One second looks like: {:?}", _one_second);    //let _this_is_true = (_one_second == _one_second);  
    // DNC: `Seconds` can't be printed; it doesn't implement the `Debug` trait    // DNC: `Seconds` can't be compared; it doesn't implement the `PartialEq` trait  
    let foot = Inches(12);  
